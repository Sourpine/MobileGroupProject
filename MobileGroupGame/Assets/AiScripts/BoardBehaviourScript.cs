...
public class BoardBehaviourScript : MonoBehaviour
{
    ...
    void AIGetPlacing()
    {
        AIGameState InitialState = new AIGameState(MyTableCards, AIHandCards, AITableCards, MyHero, AIHero, maxMana, MyMana, AIMana, turn, null);
    ...
    void AIGetAttacks()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (a.OpCode == 1)
            {
                foreach (var item in AITableCards)
                {
                    if (item.GetComponent<CardBehaviourScript>()._name == a.Card1)
                    {
                ...
                foreach (var item in MyTableCards)
                {
